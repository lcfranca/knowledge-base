...
pacman::p_load("tinytex", "tidyverse", "xtable", "knitr", "matlib", "MASS")
...
One of the most important concepts is the **inverse** of a matrix. This concept is very important for solving a system of linear equations.

**Definition** Suppose `A` is a square matrix, _i.e._, an $m x m$ matrix. Then the **inverse** of a matrix $A$ is an $m x m$ matrix $A^{-1}$ such that
...
In `R`, if we want to compute the inverse of a matrix we can use the `ìnv()` function from the `matlab` package. For example, if we have a matrix 

\begin{equation*}
A = \left(
```{r echo=F, results="asis"}
x <- matrix(c(1,-2,-1,2,3,2,3,-2,1), nrow = 3, ncol = 3)
xtable::xtableMatharray(x)
```
\right).
\end{equation*}
...
If we want to see the output in terms of rational numbers, then we can use the `fractions()`function from the `MAAS`package: 

```{r}
library(MASS)
MASS::fractions(inv(A))
```
In `R` we can also use the `solve()` function to find the inverse of a matrix instead of the `ìnv()` function. For example: 
